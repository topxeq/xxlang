...
| 基准测试 | 栈式 VM | 寄存器 VM | 加速比 |
|----------|---------|-----------|--------|
| Fibonacci(15) | 971 µs | 52 µs | **18.6 倍** |
| 阶乘 | 400 µs | 52 µs | **7.7 倍** |
| 函数调用 | 534 µs | 78 µs | **6.8 倍** |
| 循环(1000) | 687 µs | 267 µs | **2.6 倍** |
...
|------|------|---------|
| **Go** | **4.8 ms** | 1x (基准) |
| Python | 247 ms | 慢 51 倍 |
| Xxlang | 559 ms | 慢 116 倍 |
...
**关键发现：**
- 寄存器 VM 比栈式 VM 快 **2-18 倍**
- TCO 使递归比朴素方法快 **400,000 倍**
- Xxlang TCO 可以**匹配甚至超越** Go 的迭代实现

详见 [tests/benchmark_results.md](tests/benchmark_results.md)。
