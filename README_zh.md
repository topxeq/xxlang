...
- **服务模式** - 作为系统服务运行（Windows/Linux/macOS），自动启动，后台任务执行
- **插件系统** - WASM 插件实现高性能操作（兼容 Windows，无需 CGO）
- **丰富的内置函数** - 460+ 个内置函数，支持字符串、数学、数组、映射、HTTP、并发、加密等
...
Xxlang 使用现代的基于寄存器的虚拟机：

```bash
xxl script.xxl                  # 使用寄存器 VM（默认）
```
...
Xxlang 使用基于寄存器的字节码虚拟机，支持尾调用优化，性能优异。

### 寄存器 VM vs Go vs Python

| 基准测试 | 寄存器 VM (µs) | Go (µs) | Python (µs) |
|----------|---------------:|--------:|------------:|
| Fibonacci(15) | 872 | 3.5 | 178 |
| Fibonacci(20) | 4,785 | 39 | 1,980 |
| Fibonacci 迭代(30) | 403 | ~0.01 | 1.6 |
| 阶乘(12) | 388 | ~0.01 | 1.4 |
| For 循环(100) | 382 | ~0.04 | 3.7 |
| **For 循环(1000)** | **474** | ~0.33 | 43.7 |
| While 循环(100) | 348 | ~0.04 | 6.5 |
| 函数调用(200) | 526 | ~0.04 | 17.7 |
| 算术运算 | 338 | ~0.0003 | 0.15 |
| **密集算术(1000)** | **595** | ~0.65 | 139 |
| 嵌套表达式 | 360 | ~0.0003 | 0.13 |
| 比较运算(400) | 472 | ~0.04 | 12.7 |
| **素数检测(100)** | **481** | 2.4 | 31 |
| 冒泡排序(10) | 455 | ~0.04 | 6.7 |
| 数组求和(1000) | 353 | ~0.67 | 16.1 |
| **字符串拼接(100)** | **324** | 5.2 | 5.5 |
