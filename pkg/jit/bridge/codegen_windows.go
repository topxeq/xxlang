...
// This version uses Microsoft x64 ABI (Windows).
// Arguments: rcx = n, returns result in rax.
func BuildFibCode() []byte {
	code := make([]byte, 0, 128)

	// Save callee-saved registers (Microsoft x64 ABI)
	...
	// Save n to r13 (arg is in rcx for Windows, NOT rdi!)
