...
func ProcessXHP(content, scriptPath string, res http.ResponseWriter, req *http.Request,
	...
	for {
		...
		if startIdx == -1 {
			// No more XHP blocks, append remaining content
		...
		output, err := RunScriptOnHttp(code, scriptPath, res, req, paraMap, globals)
		if err != nil {
			return "", fmt.Errorf("XHP code error at position %d: %v", startIdx, err)
		}
