...
func TestBase64Encoding(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if encoded != tt.expected {
			t.Errorf("encodeBase64(%q) = %q, want %q", tt.input, encoded, tt.expected)
		}
		...
		if tt.input != "" {
			...
			if string(decoded) != tt.input {
				t.Errorf("decodeBase64 round trip failed: got %q, want %q", string(decoded), tt.input)
			}
		}
	}
}
...
func TestDecodeBase64Invalid(t *testing.T) {
	...
	_, err = decodeBase64("abc") // Length not multiple of 4
	if err == nil {
